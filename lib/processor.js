...
exports = module.exports = function processor(FfmpegCommand) {
      ...
      self._prepare(function(err) {
        ...
        self.ffmpegProc.stderr.on('data', function (data) {
          ...
          if (self.listeners('progress').length) {
            var duration =  0
            if (self.metaData && typeof self.metaData.durationsec === 'number') {
                duration = self.metaData.durationsec;
            }
            self._getProgressFromStdErr(stderr, duration);
          }
        });
      });
    }
  ...
  FfmpegCommand.prototype._checkStdErrForCodec = function(stderrString) {
    ...
    var codecInfoPassed = /Output #0/.test(stderrString);
    if (codecInfoPassed) {
